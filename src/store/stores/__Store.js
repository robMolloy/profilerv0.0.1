...
class Store {
  ...
  get defaultGetters() {
    return {
      ...
      indexOnKey: state => key => {
        return index(state).on(key);
      },
      groupByKey: (state, getters) => key => {
        return group(getters.data).by(key);
      },
      groupValuesByKey: (state, getters) => (key, orderKey) => {
        ...
        Object.entries(groups).forEach(([key1, group]) => {
          const valuesOnKey1 = Object.values(group);
      ...
      groupByUid: (state, getters) => uid => {
        return getters.groupByKey("uid")[uid];
      },
